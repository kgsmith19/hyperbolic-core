...
| T-A-001 | `tests/rls.test.mjs` (page load) | acceptance | AC-001 -> FR-001 | The list page fails to render a seeded row's fields | Only an end-to-end page load proves the browser contract | No other test loads the actual HTML page | pending (no page exists yet; see task to build idea-list UI) | | Page replaced by a different tool's UI | SL-000 |
