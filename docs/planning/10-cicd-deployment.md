...
**RESOLVED.** It was Tailscale Funnel: the tailnet ACL carried a standing `{"target": ["100.81.145.92"], "attr": ["funnel"]}` node-attribute grant for `lifeos-prod`, separate from and in addition to the generic `autogroup:member` starter-policy grant, confirmed via the Tailscale API. The grant has been removed from the live ACL directly [VERIFIED: `GET tailnet/-/acl` re-fetched after the update shows no `100.81.145.92` entry], which closes ADR-06's public-exposure question without requiring the release-smoke.yml job conversion this section originally recommended. `docs/ops/tailscale-serve-apply.sh`'s `AllowFunnel` check (both `validate_initial_json` and `verify_gateway_json`) was also hardened: it previously only validated the field's *type* and silently accepted a truthy Funnel grant as a passing state; it now fails closed whenever `AllowFunnel` is anything other than `False`/`None`/`{}`, so this can no longer regress undetected. Whether `release-smoke.yml` itself still lacks a tailnet-join step remains true and is unchanged by this fix -- that workflow still lives in the standalone lifeos repo, out of this monorepo's reach -- but the exposure it relied on no longer exists at the infrastructure layer.
