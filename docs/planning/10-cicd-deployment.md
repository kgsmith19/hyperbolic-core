...
- Destructive migrations (drop, irreversible rewrite, PII erasure) require a backup-first step: the deploy is blocked until a fresh backup artifact exists, i.e. run the backup workflow and record its run id in the PR before merge. The platform-project pipeline is `.github/workflows/platform-backup.yml` (m6-03), which extends the age-encrypted pattern to the platform project's own schemas. The operative rule now lives in `docs/ops/runbook.md`, "Platform project backup and restore": a destructive platform migration PR must cite the run id of a backup run that completed after the PR's base commit, since a backup taken before that commit does not cover the rows the migration removes.
...
**RESOLVED.** It was Tailscale Funnel: the tailnet ACL carried a standing `{"target": ["100.81.145.92"], "attr": ["funnel"]}` node-attribute grant for `lifeos-prod`, separate from and in addition to the generic `autogroup:member` starter-policy grant, confirmed via the Tailscale API. The grant has been removed from the live ACL directly [VERIFIED: `GET tailnet/-/acl` re-fetched after the update shows no `100.81.145.92` entry], which closes ADR-06's public-exposure question without requiring the release-smoke.yml job conversion this section originally recommended. `docs/ops/tailscale-serve-apply.sh`'s `AllowFunnel` check (both `validate_initial_json` and `verify_gateway_json`) was independently re-verified against this ACL fix: it already fails closed whenever `AllowFunnel` is present and is not an empty object -- `for field in ("Services", "AllowFunnel", "Foreground"): if field in state: if not isinstance(state[field], dict) or len(state[field]) > 0: fail(...)` -- so a live Funnel grant (bool `true` or a populated per-host dict) was never going to pass this script silently; no code change to it was needed or made. Whether `release-smoke.yml` itself still lacks a tailnet-join step remains true and is unchanged by this fix -- that workflow still lives in the standalone lifeos repo, out of this monorepo's reach -- but the exposure it relied on no longer exists at the infrastructure layer.
