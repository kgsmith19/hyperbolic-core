...
- Destructive migrations (drop, irreversible rewrite, PII erasure) require a backup-first step: the deploy is blocked until a fresh backup artifact exists. `.github/workflows/platform-backup.yml` (m6-03) extends the age-encrypted backup pattern to the platform Supabase project; the rule is: dispatch that workflow, wait for the run to complete, and record its run id in the PR description before merge. A destructive platform migration PR with no recorded run id is refused -- full rule text and the restore-drill record live in `docs/ops/runbook.md`'s "Platform backup" section.
