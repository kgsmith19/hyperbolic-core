...
| `prompt.prompt` | uuid PK, user_id FK auth.users default auth.uid(), title check 1-200 + unique `(user_id, lower(title))`, body check 1-100000, created_at, is_active | [VERIFIED: apps/toolbelt/apps/prompt-organizer/supabase/migrations/20260807020000_prompt_create_prompt.sql:7-13; 20260813151000; 20260808000000:3] | RLS re-pin per 05-d section 2; new index (user_id, created_at desc) |
...
| `prompt.get_prompt_source` RPC | NEW | 05-d sections 1.2 and 4 [VERIFIED: docs/planning/05-d-prompt-organizer.md] |
...
`prompt.render_prompt` remains security invoker, so pinned table policies constrain it. `prompt.get_prompt` and `prompt.get_prompt_source` are SECURITY DEFINER because `prompt_get_agent` is execute-only; each performs the owner/scoped-token authorization check before any read, uses an empty `search_path`, and exposes no table privilege [VERIFIED: their migrations; docs/planning/05-d sections 1.2 and 4].

### 5.6 Latency note: RLS function-call overhead on the hot render path

The hot path is prompt injection: `rpc/get_prompt` and the client's conditional `rpc/get_prompt_source` are budgeted at 60 ms p50 / 150 ms p95 [VERIFIED: docs/planning/05-d section 4]. Every owner-scoped read evaluates `platform.owner()`. Budget and mitigation:
...
| --- | --- | --- | --- | --- |
| Q1 | Prompt list: `GET /rest/v1/prompt?select=...&order=created_at.desc` with embedded tag/version/configuration | [VERIFIED: apps/toolbelt/apps/prompt-organizer/web/index.html:171-173] | nothing (PK uuid, owner-scoped unique title only) | ADD `prompt_created_at` below |
...
| Q7 | `get_prompt` / `get_prompt_source` / `render_prompt` resolve by owner plus `lower(title)` (the invoker RPC receives the same owner predicate through RLS) | [VERIFIED: RPC migrations; 05-d section 1.2] | unique index `prompt_title_unique` on `(user_id, lower(title))` [VERIFIED: 20260813151000] | none needed |
...
**Directory decision: keep per-app source directories, but stage one temporary CLI migration tree.** Schema ownership still follows tool ownership, and the 05-c scaffold still generates per-tool `supabase/migrations/` [VERIFIED: docs/planning/05-c section 5.1 layout]. Before any push, validation discovers the Toolbelt root plus every schema-owning manifest, requires globally unique version keys and paired downs, and copies only forward files into one version-sorted temporary `supabase/migrations/` directory. Schema-less tools are excluded. This preserves ownership without pretending that one physical database has separate migration ledgers.

**Mechanism decision: exactly one `supabase db push` over the staged global ledger, from `.github/workflows/platform-migrations.yml`.** A per-directory push is invalid because the Supabase CLI compares each invocation with the same `supabase_migrations.schema_migrations` ledger, and a paired `_down.sql` placed in its input directory is otherwise indistinguishable from a forward migration. The psql-loop flavor is also rejected because it re-executes non-idempotent DDL. Concrete design:

- The root workflow is callable by the main-branch deploy pipeline and explicitly dispatchable for the reviewed one-time adoption. `concurrency: toolbelt-db` serializes applies; it refuses any database mutation unless the checked-out SHA is still the current `main` SHA.
- The PR gate performs the same dynamic discovery, manifest/registry parity, global-version, paired-down, reserved-schema, and owner-policy validation, then proves the staged tree contains forward files only.
- Auth uses GitHub OIDC to an Infisical machine identity scoped to `/platform/`, yielding `SUPABASE_DB_URL` without printing it.
- Normal apply requires the remote ledger to be an exact prefix of the local staged ledger, verifies `platform.config` resolves to exactly one confirmed and unbanned owner, diffs the schema represented by the already-applied prefix, runs `supabase db push --dry-run`, rechecks the immutable main SHA, performs one `supabase db push --include-all --yes`, then runs a second dry run to prove nothing remains pending.
- One-time adoption is explicit and restartable. It accepts only an exact prefix of the reviewed 18 legacy versions plus the two additive S1 bootstrap versions. Before repairing legacy ledger rows it requires an empty schema diff and separately verifies legacy seed payloads, dependency/status backfills, prompt version history, grants, exposed schemas, retention functions, and the pg_cron job -- effects a schema diff cannot prove. It then applies only S1 and stops for owner provisioning; owner-dependent migrations require a later normal run.
- `project.yaml` `apply_migration` points operators at this workflow, retiring the by-hand procedure. A scaffolded tool needs no workflow allowlist edit: its schema-owning manifest makes its directory discoverable.
