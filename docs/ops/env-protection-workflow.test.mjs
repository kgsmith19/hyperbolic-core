...
function jobBlock(workflow, jobKey, nextJobKey) {
  ...
  const end = nextJobKey ? workflow.indexOf(`\n  ${nextJobKey}:`, start + 1) : workflow.length;
  assert.ok(nextJobKey ? end > start : true, `job "${nextJobKey}" not found (used as end marker)`);
...
test("deploy.yml: every deploy job carries its own correctly-named production environment", () => {
  assert.match(jobBlock(deploy, "deploy-shell", "build-llm-handler"), /environment: shell-deploy-production/);
  assert.match(jobBlock(deploy, "deploy-llm-handler", "build-brain"), /environment: llm-handler-deploy-production/);
  assert.match(jobBlock(deploy, "deploy-brain", "smoke"), /environment: brain-deploy-production/);
});
...
test("deploy.yml: migrate-platform picks up platform-migrations-production through the reusable workflow it calls, not a second declaration on the caller job", () => {
  ...
  assert.match(migrateCaller, /uses: \.\/\.github\/workflows\/platform-migrations\.yml/);
});
...
test("lifeos-deploy.yml: both deploy jobs carry their own correctly-named production environment", () => {
  assert.match(jobBlock(lifeosDeploy, "deploy-backend", "deploy-ui"), /environment: lifeos-backend-deploy-production/);
  assert.match(jobBlock(lifeosDeploy, "deploy-ui"), /environment: lifeos-ui-deploy-production/);
});
...
test("platform-backup.yml and lifeos-backup.yml: the bundle job carries its own production environment", () => {
  assert.match(jobBlock(platformBackup, "bundle"), /environment: platform-backup-production/);
  assert.match(jobBlock(lifeosBackup, "bundle"), /environment: lifeos-backup-production/);
});
...
test("ops-serve-apply.yml and ops-edge.yml: both SSH-into-and-mutate-the-box jobs carry their own production environment", () => {
  assert.match(jobBlock(opsServeApply, "apply"), /environment: ops-serve-apply-production/);
...
test("adding an environment: does not remove any existing vars.*_ENABLED gate -- environments are additive defense in depth, not a replacement", () => {
  assert.match(jobBlock(deploy, "deploy-shell", "build-llm-handler"), /vars\.DEPLOY_ENABLED == 'true'/);
  assert.match(jobBlock(deploy, "deploy-llm-handler", "build-brain"), /vars\.DEPLOY_ENABLED == 'true'/);
  assert.match(jobBlock(deploy, "deploy-brain", "smoke"), /vars\.DEPLOY_ENABLED == 'true'/);
  assert.match(jobBlock(lifeosDeploy, "deploy-backend", "deploy-ui"), /vars\.LIFEOS_DEPLOY_ENABLED == 'true'/);
  assert.match(jobBlock(lifeosDeploy, "deploy-ui"), /vars\.LIFEOS_DEPLOY_ENABLED == 'true'/);
