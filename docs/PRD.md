...
| FR-008 | The system must save a named configuration consisting of variable values and a list of included section ids, and must apply it on selection. | Must | Given a configuration `lean` with `REPO=toolbelt` and sections `[a]`, when `lean` is selected, then the variable field for `REPO` reads `toolbelt` and only section `a` is included. | UC-003 | done |
...
| FR-013 | The system must serve a rendered prompt over an authenticated HTTP endpoint given a prompt name and a configuration name. | Could | Given `GET /rest/v1/rpc/render_prompt?p_name=spec-author&p_config=lean` with a valid Bearer token, then the response body is the rendered text (JSON-quoted) with content type `application/json` and status `200`. (Corrected 2026-08-08, SPEC-0012: the exact path and `text/plain` claim were written before a mechanism was chosen; PostgREST's raw-text output needs a server config this managed project doesn't expose, confirmed live — see SPEC-0012 section 2.1.) | U-002 | done |
...
|---|---|---|---|---|---|
| NFR-001 | Performance | Search must return results within 300 ms at p95 with 1,000 prompts stored. | 300 ms | Timed unit benchmark (`tests/performance.test.mjs`, T-U-029) over 1,000 synthetic prompts — corrected from "timed query against a seeded database": search runs client-side over the already-fetched list (`docs/DATA-FLOW-DIAGRAM.md` F-4), never a database query | done |
...
| NFR-004 | Security | No API key or service-role credential may appear in client-delivered code. | 0 occurrences | Grep of the repository — corrected from "CI" grep: no CI and no build exist, deliberately (SR-17), so there is no built bundle to grep; the source files ship as-is | done |
...
| NFR-006 | Availability | The tool must be usable whenever the database is reachable. No additional service may be required to render. | Rendering has zero external dependencies | Inspection: `web/render.mjs` contains no `fetch`, no `await`, no I/O of any kind | done |
| NFR-007 | Cost | Infrastructure cost must be $0 above the existing `toolbelt` project. | $0 marginal | Billing report | done |
| NFR-008 | Accessibility | Every control must be reachable and operable by keyboard alone, and search must be focusable by a single keystroke from anywhere in the app. | 0 keyboard traps | Inspection: every control is a native `button`/`input`/`select`/`details`, inherently keyboard-operable; 0 `tabindex` or focus-stealing code beyond the two ordinary form-submit guards. `/` focuses search from anywhere except an already-focused text field (`tests/search.test.mjs`, T-U-030) | done |
...
| NFR-010 | Observability | Every render must write one `core.run` row and one `core.cost` row with wall-clock time. | 100% of renders | Row count versus usage count | done |
| NFR-011 | Privacy | Prompt bodies must never be sent to any third party by this application. | 0 outbound calls carrying body text | Inspection: `web/index.html` contains exactly 2 `fetch` targets, both the Supabase project (SR-08) | done |
...
| SL-005 | Configurations | Named sets of variable values plus included sections are saved and applied. | FR-008 | 55 (shipped 2026-08-08, SPEC-0011) | SL-003, SL-004 |
...
| SL-009 | Render endpoint | An authenticated HTTP endpoint returns rendered text by name and configuration. | FR-013 | 45 (shipped 2026-08-08, SPEC-0012) | SL-005 |
...
| 2026-08-08 | 0.1.14 | SL-009 shipped (SPEC-0012): FR-013 → `done` — the last open PRD requirement. `prompt.render_prompt(p_name, p_config)`, a `security invoker`, `stable` PostgREST RPC (SR-02: not a new service — more `prompt` schema exposed through the PostgREST layer already used everywhere else), reusing FR-010's missing-variable guard rather than re-deciding it. FR-013's own AC corrected: `text/plain` isn't available on this managed project (PostgREST's raw output needs a server config not exposed to migrations, confirmed live); the endpoint returns `application/json` like everything else here. A real bug found and fixed during mutation testing: Postgres's default `PUBLIC` execute grant was masking the intended `authenticated`-only surface. | Directed to continue FR-013 and pick the mechanism; the same call NFR-010 already set precedent for (an RPC, not a new service) applied here too. Every PRD requirement is now `done`. | FR-013, SL-009 |
| 2026-08-08 | 0.1.13 | NFR-008 → `done`. Every existing control was already keyboard-operable by virtue of being a native HTML element (no custom widgets exist); the one real gap was "search focusable by a single keystroke," closed with a 4-line `shouldFocusSearch` (`/` focuses search, except while a text field already has focus) plus one `keydown` listener. | Same pattern as the other NFR closures this session: check what's actually missing before writing code. Here something genuinely was missing, so it got the standard red-green treatment (T-U-030, mutation-verified) rather than a docs-only closure. | NFR-008 |
| 2026-08-08 | 0.1.12 | NFR-001 → `done` (T-U-029, a new benchmark, `tests/performance.test.mjs`). NFR-004, NFR-006, NFR-011 → `done` by direct inspection, no new code: no service-role credential exists anywhere in the repo (only the Postgres role name in a grant statement); `render.mjs` has zero I/O; exactly 2 `fetch` targets exist, both Supabase. All three "how measured" columns corrected where stale (NFR-001, NFR-004 both referenced a database query / CI build that this repo's architecture never had). | Several NFRs were sitting at `not-started` not because anything was missing, but because nobody had gone and checked — the requirements were already true by the existing minimal design. Closing them with real evidence rather than new code is the same "cheapest sufficient mechanism" ladder CLAUDE.md already states; a schema-constraint-cheap answer beats writing code to prove something already provably true. | NFR-001, NFR-004, NFR-006, NFR-011 |
| 2026-08-08 | 0.1.11 | SL-005 shipped (SPEC-0011): named configurations. `prompt.configuration` reuses `prompt.tag`'s exact ownership shape (composite PK, no owner column, RLS via `EXISTS` against the parent row) rather than a new pattern. FR-008 (last remaining `Must`-priority requirement) → `done`. | Asked to keep slices as lean as possible and to reuse existing patterns over duplicating them; this slice shipped at ~55 LOC against the PRD's own original 280 LOC estimate by reusing `tag`'s schema shape and narrowing the grant surface to `SELECT`/`INSERT` only (update/delete not asked for by FR-008's AC). | FR-008, SL-005 |
