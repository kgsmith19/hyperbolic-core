...
See `apps/prompt-organizer/TEST_LEDGER.md`, `apps/network-checker/TEST_LEDGER.md`, and `apps/idea-intake/TEST_LEDGER.md` for those apps' own suites. Idea Intake's suite (independent security review, Finding 39) is now also run by `Toolbelt` itself, against a `postgres:` service container, not just locally.
