...
4. Open a pull request and let `Verify: Tests (Linux)` (the root's `pr-verify.yml`) verify the change.
...
A change is ready when its acceptance criteria are satisfied, relevant documentation is accurate, applicable local checks pass, and the pull request's `Verify: Tests (Linux)` reports success. State any unverified item explicitly.
