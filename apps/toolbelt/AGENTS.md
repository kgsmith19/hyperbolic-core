...
4. Open a pull request and let `Verify: Toolbelt` (the root's sequential `pr-verify.yml` chain) verify the change.
...
A change is ready when its acceptance criteria are satisfied, relevant documentation is accurate, applicable local checks pass, and the pull request's `Verify: Toolbelt` reports success. State any unverified item explicitly.
