...
4. Open a pull request and let `Toolbelt` (the root's `pr-verify.yml`) verify the change.
...
A change is ready when its acceptance criteria are satisfied, relevant documentation is accurate, applicable local checks pass, and the pull request's `Toolbelt` reports success. State any unverified item explicitly.
