...
def _persist_apply(conn, host_id, row, outcome):
    ...
    if reason == "apply_failed":
        ...
        return 4
    return _persist_rollback(conn, host_id, row, (applied_at, result, final_status))
...
def _persist_rollback(conn, host_id, row, resolution):
    ...
    _record_config_item(conn, host_id, row, final_status)
