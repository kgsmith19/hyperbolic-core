...
- [x] Maintenance follow-up (Issue #11) rerun attempted 2026-08-08 in a real Chromium session from this sandbox and still blocked by environment egress (`ERR_NAME_NOT_RESOLVED` / `TypeError: Failed to fetch` while signing in to the live Supabase host). The archive UI behavior remains API-verified but browser-unverified in this execution environment.
