-- m4-03-feat-po-injection-rpc (05-d-prompt-organizer.md sections 1.2 and 6).
-- The injection API. Sits beside prompt.render_prompt (20260808130000),
-- which stays untouched for backward compatibility with its own endpoint
-- tests. get_prompt supersedes it for new consumers by adding the three
-- things injection needs and render_prompt lacks: version pinning, ad-hoc
-- values, and provenance (version_no, rendered_at) in the response.
--
-- security invoker (the default -- no security definer clause) inherits
-- the caller's own RLS, same posture as render_prompt: no new policy is
-- added or needed, and the function cannot read a prompt/prompt_version
-- row the caller's own owner-pinned RLS (20260812180000) would not already
-- let it read directly. EXECUTE is revoked from PUBLIC and re-granted to
-- authenticated only, matching render_prompt's exact grant shape.
