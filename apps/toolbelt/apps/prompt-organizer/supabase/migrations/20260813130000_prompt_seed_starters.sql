...
--
-- `on conflict (lower(title)) do nothing` (matching prompt_title_unique,
-- 20260807041000) makes this safe to re-run and safe against a
-- pre-existing personal prompt that happens to share a seeded title: the
-- insert for that one row is skipped, the personal row is left exactly as
-- it was, and every other row still seeds normally.
--
...
('brain/task-contract', $b1$You are the Brain's dispatcher. Produce a single task contract for a coding harness run, in the exact JSON shape below. Do not include any prose outside the JSON.
...
('coding/system/kernel-run', $b2$You are Claude Code, running inside the Agentic Command Center's kernel for {{REPO}}. You operate autonomously against the task contract you were dispatched with; there is no human watching this session in real time.
...
('coding/review/simplification', $b3$You are running the code_simplification review pass over the diff below for {{REPO}}. Your only job is reuse, simplification, and efficiency; correctness bugs are out of scope for this pass (a separate security_review pass covers those).
...
('planning/spec/issue-outcome', $b4$Turn the rough outcome below into one GitHub Issue draft for {{REPO}}, following this repository's Issue-first working model.
...
('intake/optimize/idea', $b5$Sharpen the idea below into a submission-ready form. Do not invent facts not present in the input; where information is missing, note the gap instead of guessing.
...
('lifeos/chat/system', $b6$You are the LifeOS assistant. You help {{USER_NAME}} manage tasks, notes, and daily planning inside LifeOS. You have access only to the tools LifeOS has wired to this session; never claim to take an action you were not given a tool for.
...
('research/deep-dive', $b7$Research {{TOPIC}} and produce a deep-dive summary for {{AUDIENCE}}.
...
('ops/runbooks/deploy-verify', $b8$Run the post-deploy verification for {{SERVICE}} at {{ENVIRONMENT}}, deployed as {{RELEASE_REF}}.
...
('idea-intake/optimize-v1', $b9$Optimize the idea below into a submission-ready draft for {{TARGET_REPO}}. Do not invent facts the input does not support; where you had to infer or fill a gap, lower confidence accordingly rather than presenting a guess as fact.
