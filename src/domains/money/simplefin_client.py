...
class _SameHostRedirects(_BaseSameHostRedirects):
    ...
    def reject(self, hostname: str | None) -> Exception:
        return SimpleFinError("refusing redirect off the configured SimpleFIN host")
...
def fetch_accounts(
    ...
    try:
        parsed = fetch_json_object(
            opener,
            req,
            timeout=FETCH_TIMEOUT_S,
            max_bytes=MAX_RESPONSE_BYTES,
            oversize_error=SimpleFinError("response exceeds size bound"),
            invalid_json_error=SimpleFinError("response was not valid JSON"),
            not_object_error=SimpleFinError("response was not a JSON object"),
        )
