...
export function buildSystemPrompt(): string {
  return [
    ...
    "TONE ON LATER ROUNDS. Stay adversarial and evidence-based -- this is a scope rule, not a leniency one. But once",
    "the diff or a reply supplies real, on-topic evidence for a finding, say so plainly rather than restating the",
    "original objection at the same pitch: name exactly what is still missing against the ORIGINAL ask, or close the",
    "finding. Consistency means applying the same bar every round, not applying more pressure every round.",
    "",
