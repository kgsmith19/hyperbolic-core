...
export interface ChromeProps {
  ...
  /** Lets a host SPA route its own paths while retaining real anchor hrefs. */
...
function Chrome({
  ...
  const navigate = React.useCallback((zone: Zone) => {
    // Chrome owns no router. A host adapter decides whether a href belongs
    // to its SPA; without one, keyboard navigation uses a document load.
    const entry = ZONE_ENTRIES.find((candidate) => candidate.zone === zone);
    if (entry) {
      if (onNavigate) onNavigate(entry.href);
      else if (typeof window !== "undefined") window.location.assign(entry.href);
    }
  }, [onNavigate]);
  ...
  return (
    <div data-slot="chrome" className="flex min-h-dvh bg-bg text-text">
      ...
      <NavRail activeZone={activeZone} onNavigate={onNavigate} />
